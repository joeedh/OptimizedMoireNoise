...
], function fft(util, vectormath, cconst, report) {
  'use strict';
...
//      this.fft_pointcachefmax = new util.hashtable();
  ...
  var FFT1D = exports.FFT1D = Class([
    function constructor(totpoint) {
      this.samples = [];
      this._dft = new Float64Array(totpoint);
      this._dft.fill(0, 0, this._dft.length);
    },
    ...
    function add_samples(samples) {
      for (var i=0; i<samples.length; i++) {
        this.samples.push(samples[i]);
      }
    },
    ...
    function dft() {
      ...
      for (var i=0; i<this.samples.length; i++) {
        //this.samples[i] = 0.15*(cos(i*3.234234) + cos(i*0.234324) + cos(i*54.2445324) + cos(i*5.02342));
      }
      ...
      for (var i=0; i<this.samples.length; i++) {
        ...
        for (var j=0; j<this.samples.length-1; j++) {
          var s = this.samples[j];
          
          var e = -2*Math.PI*j*(i/this.samples.length);
        ...
        if (i%100 == 0) {
          console.log("doing sample ", i, "of", this.samples.length);
          console.log(sumx, sumy);
        }
        
        this._dft[i] = 0.5*sum //this.samples.length;
        
        min = Math.min(this._dft[i], min);
        max = Math.max(this._dft[i], max);
      }
      
      for (var i=0; i<this.samples.length; i++) {
        this._dft[i] = (((this._dft[i]-min) / (max - min))-0.5)*0.5;
      }
    },
    
    function normalize() {
      var tot = 0, min=1e17, max=-1e17;
      
      for (var i=0; i<this.samples.length; i++) {
        tot += this.samples[i], max = Math.max(Math.abs(this.samples[i]), max);
        min = Math.min(this.samples[i], min);
      }
    ...
    function draw(g) {
      ...
      for (var i=0; i<this._dft.length; i++) {
        var x = i/this._dft.length, y = this._dft[i];
